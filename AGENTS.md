...
- **Repo:** `github.com/lbockenstedt/opnsense`
- **Module type:** `module_type = "firewall"`
- **Canonical docs:** [`lm/docs/opnsense.md`](../lm/docs/opnsense.md) *(in the `lm` repo — the master registry)*
...
`src/opn_spoke.py` (spoke), `src/control_plane.py` (hub link), `src/opnsense_engine.py`
(the actual firewall API work). `API_SPEC.md` documents the surface.

## opnsense-specific gotchas

- `HUB_URL` defaults to `auto` — the spoke **rediscovers the hub on every connect**, unlike most modules.
- This is a **cert distribution target**: the hub pushes material from the `le` spoke here via a generic `INSTALL_CERT` command.
- `opnsense.log` is a committed runtime artifact, not source.
